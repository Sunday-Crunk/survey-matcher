...
const secureCookies = hostedMode;
...
function ensureDataRoot() {
  ...
  if (!fs.existsSync(path.join(seedOutputsDir, "matcher_review.sqlite"))) {
    throw new Error(`Could not find seed database at ${seedOutputsDir}.`);
  }
  copyDirectoryContents(seedOutputsDir, outputsDir);
}
...
function tableColumns(database: Database, tableName: string) {
  return new Set(rows<{ name: string }>(database, `PRAGMA table_info(${tableName})`).map((column) => column.name));
}

function addColumnIfMissing(database: Database, tableName: string, columnName: string, definition: string) {
...
function createSchema(database: Database) {
  database.run(`
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_decisions_one_active_response ON decisions(response_id) WHERE undone_at IS NULL;
    CREATE UNIQUE INDEX IF NOT EXISTS idx_decisions_one_active_matched_child ON decisions(roster_child_id) WHERE undone_at IS NULL AND action = 'matched' AND roster_child_id IS NOT NULL;
  ...
  `);
  addColumnIfMissing(database, "decisions", "created_by", "TEXT");
  addColumnIfMissing(database, "decisions", "undone_by", "TEXT");
  addColumnIfMissing(database, "roster_additions", "created_by", "TEXT");
}

function ensureBootstrapUser(database: Database) {
  const userCount = countSql(database, "SELECT COUNT(*) AS count FROM app_users");
...
function getRequestUser(request: http.IncomingMessage): AuthUser | null {
...
function requireUser(request: http.IncomingMessage): RequestContext {
  const user = getRequestUser(request);
  if (!user) throw new ApiError(401, "Sign in to continue.");
  return { user };
}

function loginUser(username: string, password: string, response: http.ServerResponse) {
  const user = one<AuthUser & { password_hash: string }>(
    getDb(),
...
function logoutUser(request: http.IncomingMessage, response: http.ServerResponse) {
  const token = parseCookies(request.headers.cookie).get(sessionCookieName);
  if (token) {
    run(getDb(), "UPDATE app_sessions SET revoked_at = ? WHERE token_hash = ? AND revoked_at IS NULL", [
...
async function initDatabase() {
  ensureDataRoot();
  if (!SQL) {
    SQL = await initSqlJs({ locateFile: locateSqlJsFile });
  }
...
function recordDecision(payload: RecordDecisionPayload, context: RequestContext) {
  const database = getDb();
  const now = new Date().toISOString();
  ...
  try {
    ...
    const activeDecision = one<{ id: number; action: string; created_by: string | null }>(
      database,
    ...
    if (payload.action === "matched") {
      const existing = one<{ response_id: string; created_by: string | null }>(
        database,
    ...
    if (activeDecision) {
      run(database, "UPDATE decisions SET undone_at = ?, undone_by = ? WHERE id = ?", [
        now,
...
function undoLast(context: RequestContext) {
  const database = getDb();
  const now = new Date().toISOString();
  database.run("BEGIN TRANSACTION;");
  try {
    const last = one<{ id: number; action: string; response_id: string; roster_child_id: string | null }>(
      database,
...
function undoDecision(decisionId: number, context: RequestContext) {
  const database = getDb();
  const now = new Date().toISOString();
  database.run("BEGIN TRANSACTION;");
  try {
    const activeDecision = one<{ id: number; action: string; response_id: string; roster_child_id: string | null }>(
      database,
...
function getImportHistory() {
  const importRoot = path.join(outputsDir, "raw_imports");
  if (!fs.existsSync(importRoot)) return [];
  return fs.readdirSync(importRoot, { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .map((entry) => {
      const importDir = path.join(importRoot, entry.name);
      const rawUpload = fs.readdirSync(importDir).find((file) => file.toLowerCase().endsWith(".zip") || file.toLowerCase().endsWith(".csv")) ?? "";
      const stagingDir = path.join(importDir, "staging");
      const surveyPath = path.join(stagingDir, "deduped_survey_full_responses.csv");
      const candidatePath = path.join(stagingDir, "match_candidates.csv");
      return {
        id: entry.name,
        imported_at: entry.name.replace(/-/g, ":"),
        source: importDir,
        backup_dir: path.join(outputsDir, "import_backups", entry.name),
        raw_upload: rawUpload,
        survey_rows: fs.existsSync(surveyPath) ? Math.max(0, readCsv(surveyPath).length) : null,
        candidate_rows: fs.existsSync(candidatePath) ? Math.max(0, readCsv(candidatePath).length) : null
      };
    })
...
function insertSurveyResponse(database: Database, row: CsvRow) {
  run(
    database,
  ...
  run(database, "INSERT INTO survey_response_full_rows (response_id, raw_json) VALUES (?, ?)", [
    row.response_id,
    JSON.stringify(row)
  ]);
}

function insertCandidate(database: Database, row: CsvRow) {
  run(
    database,
...
function addRosterStudent(payload: AddRosterStudentPayload, context: RequestContext) {
  const database = getDb();
...
function buildMatchedSurveyResponseExport() {
  const matchedRows = rows<Record<string, unknown>>(
    getDb(),
    `SELECT
      d.id AS decision_id,
      d.created_at AS matched_at,
...
function buildRosterCoverageExport() {
  const coverageRows = rows<Record<string, unknown>>(
    getDb(),
    `SELECT
      r.roster_child_id,
      r.school_raw AS roster_school,
...
function buildUnmatchedRosterPupilsExport() {
  const exportRows = getPupils().filter((row) => row.status !== "matched") as ExportRow[];
  return {
    fieldnames: [
      "roster_child_id",
      "roster_file",
...
function buildUnresolvedResponsesExport() {
  const exportRows = (getResponses() as ExportRow[]).filter((row) =>
...
function buildDecisionActionExport(action: string) {
  const exportRows = (getReviewedRecords() as ExportRow[]).filter((row) => row.action === action);
  return {
    fieldnames: [
      "decision_id",
      "response_id",
...
function buildAuditExport() {
  return {
    ...
    exportRows: getAuditEvents() as ExportRow[]
  };
}

function buildSchoolProgressExport() {
  return {
    fieldnames: [
      "school_raw",
      "roster_count",
    ...
    exportRows: getSchoolSummaries() as ExportRow[]
  };
}
...
async function importRawQualtricsUpload(buffer: Buffer, filenameHeader: string | undefined, context: RequestContext) {
  ...
  fs.copyFileSync(path.join(outputsDir, "normalized_roster.csv"), path.join(stagingDir, "normalized_roster.csv"));
  ...
  const database = getDb();
  const existingIds = new Set(rows<{ response_id: string }>(database, "SELECT response_id FROM survey_responses").map((row) => row.response_id));
  ...
  database.run("BEGIN TRANSACTION;");
  try {
    for (const row of newSurveyRows) insertSurveyResponse(database, row);
    for (const row of newCandidateRows) insertCandidate(database, row);
    recordAuditEvent(database, {
  ...
  const alreadyMatchedTopCandidates = rows<{ count: number }>(
    database,
    `SELECT COUNT(*) AS count
     FROM candidates c
     JOIN decisions d
...
async function handleApi(request: http.IncomingMessage, response: http.ServerResponse, requestUrl: URL) {
  ...
  if (request.method === "POST" && route === "auth/login") {
    ...
    return sendJson(response, 200, loginUser(String(body.username ?? ""), String(body.password ?? ""), response));
  }
  if (request.method === "POST" && route === "auth/logout") {
    return sendJson(response, 200, logoutUser(request, response));
  }
  if (request.method === "GET" && route === "auth/me") {
    return sendJson(response, 200, { user: publicUser(getRequestUser(request)) });
  }

  const context = requireUser(request);
  if (request.method === "GET" && route === "init") return sendJson(response, 200, { dataRoot, dbPath });
  if (request.method === "GET" && route === "stats") return sendJson(response, 200, getStats());
  if (request.method === "GET" && route === "next-response") return sendJson(response, 200, getNextResponse(requestUrl.searchParams.get("queue") ?? "unreviewed"));
  if (request.method === "GET" && route === "candidates") return sendJson(response, 200, getCandidates(requestUrl.searchParams.get("responseId") ?? ""));
  if (request.method === "POST" && route === "record-decision") return sendJson(response, 200, recordDecision(await parseBody(request), context));
  if (request.method === "POST" && route === "undo-last") return sendJson(response, 200, undoLast(context));
  if (request.method === "POST" && route === "undo-decision") {
    const body = await parseBody(request);
    return sendJson(response, 200, undoDecision(Number(body.decisionId), context));
  }
  if (request.method === "GET" && route === "search-roster") return sendJson(response, 200, searchRoster(requestUrl.searchParams.get("query") ?? ""));
  if (request.method === "GET" && route === "roster-coverage") return sendJson(response, 200, getRosterCoverage());
  ...
  if (request.method === "GET" && route === "import-history") return sendJson(response, 200, getImportHistory());
  if (request.method === "GET" && route === "schools") return sendJson(response, 200, getSchools());
  if (request.method === "POST" && route === "add-roster-student") return sendJson(response, 200, addRosterStudent(await parseBody(request), context));
  ...
  if (request.method === "GET" && route === "export/matched") return sendAuditedCsv(response, context, "matched pupil-response export", "matched_pupil_survey_export.csv", buildMatchedSurveyResponseExport());
  if (request.method === "GET" && route === "export/coverage") return sendAuditedCsv(response, context, "roster coverage export", "roster_coverage_export.csv", buildRosterCoverageExport());
  if (request.method === "GET" && route === "export/unmatched-pupils") return sendAuditedCsv(response, context, "unmatched roster pupils", "unmatched_roster_pupils.csv", buildUnmatchedRosterPupilsExport());
  if (request.method === "GET" && route === "export/unresolved-responses") return sendAuditedCsv(response, context, "unresolved responses", "unresolved_responses.csv", buildUnresolvedResponsesExport());
  if (request.method === "GET" && route === "export/ambiguous-decisions") return sendAuditedCsv(response, context, "ambiguous decisions", "ambiguous_decisions.csv", buildDecisionActionExport("ambiguous"));
  if (request.method === "GET" && route === "export/deferred-decisions") return sendAuditedCsv(response, context, "deferred decisions", "deferred_decisions.csv", buildDecisionActionExport("deferred"));
  if (request.method === "GET" && route === "export/duplicate-decisions") return sendAuditedCsv(response, context, "duplicate response decisions", "duplicate_response_decisions.csv", buildDecisionActionExport("duplicate"));
  if (request.method === "GET" && route === "export/no-match-decisions") return sendAuditedCsv(response, context, "no-match decisions", "no_match_decisions.csv", buildDecisionActionExport("no_match"));
  if (request.method === "GET" && route === "export/audit-log") return sendAuditedCsv(response, context, "audit log", "audit_log.csv", buildAuditExport());
  if (request.method === "GET" && route === "export/school-progress") return sendAuditedCsv(response, context, "school progress summary", "school_progress_summary.csv", buildSchoolProgressExport());
