  ...
  return (
    <section className="grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_340px] overflow-hidden p-4">
      <Card className="min-h-0 gap-0 py-0">
        ...
        <CardContent className="min-h-0 overflow-auto p-0">
          <Table className="text-[13px]">
            ...
            <TableBody>
              {sorted.map((school) => (
                <TableRow key={school.school_raw} className="cursor-pointer" onClick={() => setSelectedName(school.school_raw)}>
      ...
      <aside className="ml-4 min-h-0 overflow-auto rounded-md border border-line bg-panel p-4 text-[13px]">
        <div className="text-[15px] font-semibold">{selected?.school_raw ?? "No school selected"}</div>
        {selected ? (
